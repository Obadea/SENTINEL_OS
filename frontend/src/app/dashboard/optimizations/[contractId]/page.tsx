...
export default function OptimizationReportPage({ params }: { params: Promise<{ contractId: string }> }) {
    ...
    const handleCopyCode = () => {
        const cleanCode = optimizedCode.map((line: string) => line.replace("|", "")).join("\n");
            ...
            <div className="p-8 pb-4">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div className="space-y-2">
                        ...
                        <h1 className="text-4xl font-heading font-black uppercase tracking-tighter text-on-surface">
                            {analysis.filename}
                        </h1>
                        <p className="text-on-surface-variant font-mono text-xs max-w-xl leading-relaxed uppercase tracking-wider opacity-70">
                            ...
                            and memory management for {analysis.address || "development"} execution.
                        </p>
                    </div>
                    ...
                    <div className="flex gap-3">
                        <button
                            ...
                            className="flex items-center gap-2 h-12 px-6 border border-wireframe text-on-surface font-mono text-[10px] uppercase tracking-widest hover:bg-white/5 transition-all rounded-none"
                        >
                            <IconDownload className="w-4 h-4" />
                            Download Report
                        </button>
                        <RippleButton
                            className="h-12 px-6 bg-neon-green text-on-primary font-heading font-black text-[11px] uppercase tracking-widest rounded-none shadow-[0_0_20px_rgba(161,216,0,0.2)] flex items-center gap-2"
                        >
                            <IconRocket className="w-4 h-4" />
                            Deploy via Mantle SDK
                        </RippleButton>
                    </div>
                </div>
            </div>
                        ...
                        <div className="flex flex-col bg-[#050505] min-h-full">
                            ...
                            <div className="p-6 flex-1 h-full">
                                {originalCode.map((line: string, i: number) => {
                                    ...
                                    const isChanged = trimmedLine.startsWith("|");
                                    ...
                                    return (
                                        <div key={i} className={cn(
                                            ...
                                            <span className="w-8 text-on-surface-variant/30 text-right select-none border-r border-wireframe/10 pr-2 pt-0.5">{i + 1}</span>
                                            <span
                                                className={cn("whitespace-pre pl-2 pt-0.5", isChanged ? "text-on-surface font-medium" : "text-on-surface-variant/80")}
                                ...
                                {optimizedCode.map((line: string, i: number) => {
                                    ...
                                    return (
                                        <div key={i} className={cn(
                                            ...
                                            <span className="w-8 text-neon-green/30 text-right select-none border-r border-wireframe/10 pr-2 pt-0.5">{i + 1}</span>
                                            <span
                                                className={cn("whitespace-pre pl-2 pt-0.5", isChanged ? "text-on-surface font-medium" : "text-on-surface-variant/80")}
                                                dangerouslySetInnerHTML={{ __html: highlightSolidity(cleanLine) }}
                                            />
